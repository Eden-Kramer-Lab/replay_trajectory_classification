...
try:
    ...
    def estimate_place_fields_kde_gpu(position,
                                      spikes,
                                      place_bin_centers,
                                      position_std,
    ...
    def poisson_log_likelihood(spikes, conditional_intensity):
        ...
        # spikes are 0 or 1
        return (cp.log(conditional_intensity[cp.newaxis, :] + np.spacing(1)) *
                spikes[:, np.newaxis] - conditional_intensity[cp.newaxis, :])
    ...
    def estimate_spiking_likelihood_kde_gpu(spikes, conditional_intensity,
                                            is_track_interior=None):
        """
